...
export const WorkspaceSidebar = () => {
  ...
  const channelId = useChannelId(); //fetches the current channelId...
  const memberId = useMemberId(); //fetches the current memberId selected from url path...
  ...
  const { data: member, isLoading: memberIsLoading } = useCurrentMember({
    id: workspaceId,
  }); //fetches current member details..
  const { data: currWorkspace, isLoading: currWorkspaceIsLoading } =
    useGetByIdWorkspace({ id: workspaceId });
  ...
  return (
    <div className="flex flex-col bg-[#5E2C5F] h-full">
      ...
      <WorkspaceSection label="Messages" hint="New Message" onNew={() => {}}>
        {members?.map((item) => (
          <UserItem
            key={item._id}
            id={item._id}
